...
export default class Enquiry extends React.Component {
    ...
    GetList = async() => {
      ...
      this.Load()
      LoginAPI('http://got-crm.com/api/mobile/listEnquiry.php',params,this.successcallback,this.error,this.networkissue)
    }
    successcallback=async(data)=>{
        ...
        if(data.status==1){  
         console.log('Login Response--->',data)  
         this.setState({dataSource:data.order_list})  
        // this.TodayFollowups()
        }
       else {  
          //console.log('Login Response--->',data)   
    ...
    render() {
        ...
        return (
            <SafeAreaView style={{ flex: 1 }}>
                ...
                {/* <Spinner
                    ...
                    <View style={{ justifyContent: "center", alignItems: 'center' }}>
                      <Text style={{ fontSize: 17, color: '#fff', textAlign: 'center' }}>Enquires</Text>
                    </View>
            ...
            this.state.dataSource.length!=0? this.state.dataSource.map((item, key) =>
              (
                <EnquiryComponent  key={item.order_list} onClickFunction={this.update_Layout.bind(this, key)} item={item} />
              )):
              <View>
              <View style={{justifyContent:'center',alignItems:'center'}}>
